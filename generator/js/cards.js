...
function card_data_icon(card_data, options, isBack) {
    var icon = undefined;
    if (isBack) {
        icon = card_data.icon_back;
    }
    return icon || card_data.icon || options.default_icon || "";
}
...
function card_size_class(card_data, options) {    
    var card_font_size = card_data.font_size || options.default_card_font_size || '';
...
function card_element_title(card_data, options, isBack) {
    var title = card_data.title || "";
    if (isBack && card_data.back_title && card_data.back_title.length > 0) {
        title = card_data.back_title;
    }
    var title_size = card_data.title_size || options.default_title_size || 'normal';
    if (isBack && card_data.back_title_size) {
        title_size = card_data.back_title_size;
    }
    return '<div class="card-title card-title-' + title_size + '">' + title + '</div>';
}

function card_element_icon(card_data, options, isBack) {
    var icons = card_data_icon(card_data, options, isBack).split(/[\s\uFEFF\xA0]+/).filter(icon=>icon);
    var classname = "icon";
...
function card_element_pf_action(params, card_data, options) {
    var element_class = card_element_class(card_data, options);

    var result = "";
    result += '<div class="' + element_class + '">';
    result += '   <h4 class="card-description-name"><span class="pf">' + params[0] + '</span> ' + params[1] + '</h4>';
...
function card_generate_front(data, options) {
    ...
    result += card_element_title(data, options, false);
    result += card_element_icon(data, options, false);
    result += '</div>';
...
function card_generate_back_text(data, options) {
    ...
    result += '<div class="card ' + (options.rounded_corners ? 'rounded-corners' : '') + '" ' + card_style + '>';
    result += '<div class="card-header">';
    result += card_element_title(data, options, true);
    result += card_element_icon(data, options, true);
    result += '</div>';
...
function card_generate_back(data, options) {
    ...
    else 
    {
        ...
        var icon = card_data_icon(data, options, true);
